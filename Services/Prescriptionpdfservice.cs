            ...
            var document = Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Content().Column(col =>
                    {
                        ...
                        // ── Footer ──
                        col.Item().Background(Cream).Padding(16).Row(row =>
                        {
                            row.RelativeItem().Text($"Generated by CareConnect · {DateTime.Now:dd MMM yyyy}")
                                .FontSize(7.5f).FontColor(MutedText);
                            row.RelativeItem().AlignRight()
                                .Text("This is a computer-generated document and does not require a physical stamp.")
                                .FontSize(7.5f).FontColor(MutedText);
                        });
                    });
                });
