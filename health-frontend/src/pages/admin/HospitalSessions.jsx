    ...
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "60vh", background: T.cream }}>
        <div>
          ...
          <p style={{ color: T.muted, fontSize: 14 }}>Loading hospitals & sessions…</p>
        </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.cream, padding: 28, fontFamily: "Inter, sans-serif", color: T.ink }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24, flexWrap: "wrap", gap: 14 }}>
        <div>
          <h1 style={{ fontFamily: "Fraunces, serif", fontWeight: 900, fontSize: 28, margin: 0, color: T.ink }}>Hospitals & Sessions</h1>
        ...
        <div style={card}>
          <div style={{ padding: "18px 24px", borderBottom: `1px solid ${T.border}`, background: T.cream, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontFamily: "Fraunces, serif", fontWeight: 700, fontSize: 17, color: T.ink }}>Hospitals</span>
            </div>
            <button
              onClick={openAddHospital}
              style={{ display: "flex", alignItems: "center", gap: 8, background: T.terra, color: T.white, border: "none", borderRadius: 12, padding: "10px 18px", fontWeight: 700, fontSize: 13.5, cursor: "pointer" }}
            >
                ...
                {hospitals.length === 0 ? (
                  <tr>
                    <td colSpan={5} style={{ textAlign: "center", padding: "56px 0", color: T.muted }}>
                      <Building2 size={44} style={{ opacity: 0.3, display: "block", margin: "0 auto 12px" }} />
                  ...
                  hospitals.map((h) => {
                    ...
                    return (
                      <tr key={h.id} style={{ borderTop: `1px solid ${T.border}` }}>
                        <td style={{ padding: "15px 20px", fontWeight: 700, color: T.ink }}>{h.name}</td>
                        <td style={{ padding: "15px 20px", fontSize: 13, color: T.muted }}>
                        ...
                        <td style={{ padding: "15px 20px" }}>
                          <span style={{ padding: "4px 12px", borderRadius: 99, fontSize: 12, fontWeight: 700, background: T.greenLight, color: T.green, border: "1px solid #BBD9A0" }}>
                        ...
                        <td style={{ padding: "15px 20px" }}>
                          <div style={{ display: "flex", gap: 8 }}>
                            <button
                              ...
                              style={{ padding: "6px 12px", borderRadius: 8, border: "none", cursor: "pointer", background: T.terraLight, color: T.terra, fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", gap: 5 }}
                            >
                              <Plus size={13} /> Slot
                            </button>
                            <button
      ...
      {/* Hospital modal */}
      {showHospitalModal && (
        <Modal title={editingHospital ? "Edit Hospital" : "Add Hospital"} onClose={() => setShowHospitalModal(false)}>
          <LabeledInput label="Hospital Name" value={hospitalForm.name} onChange={(e) => setHospitalForm({ ...hospitalForm, name: e.target.value })} placeholder="e.g. ABC Hospital" />
          <LabeledInput label="Address" value={hospitalForm.address} onChange={(e) => setHospitalForm({ ...hospitalForm, address: e.target.value })} placeholder="Street address" />
          <LabeledInput label="City" value={hospitalForm.city} onChange={(e) => setHospitalForm({ ...hospitalForm, city: e.target.value })} placeholder="City" />
          <LabeledInput label="Phone" value={hospitalForm.phone} onChange={(e) => setHospitalForm({ ...hospitalForm, phone: e.target.value })} placeholder="Contact number" />
          <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
            <button onClick={() => setShowHospitalModal(false)} style={{ flex: 1, padding: "11px 0", borderRadius: 10, border: `1px solid ${T.border}`, background: T.cream, color: T.ink, fontWeight: 600, fontSize: 14, cursor: "pointer" }}>
              Cancel
            </button>
            <button onClick={saveHospital} style={{ flex: 1, padding: "11px 0", borderRadius: 10, border: "none", background: T.green, color: T.white, fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
      ...
      {/* Session modal */}
      {showSessionModal && (
        <Modal title={editingSession ? "Edit Time Slot" : "Add Time Slot"} onClose={() => setShowSessionModal(false)}>
          <LabeledSelect label="Hospital" value={sessionForm.hospitalId} onChange={(e) => setSessionForm({ ...sessionForm, hospitalId: e.target.value })}>
          ...
          <LabeledSelect label="Day" value={sessionForm.day} onChange={(e) => setSessionForm({ ...sessionForm, day: e.target.value })}>
            {DAYS.map((d) => (
          ...
          <div style={{ display: "flex", gap: 12 }}>
            <div style={{ flex: 1 }}>
              <LabeledInput label="Start Time" type="time" value={sessionForm.startTime} onChange={(e) => setSessionForm({ ...sessionForm, startTime: e.target.value })} />
            </div>
            <div style={{ flex: 1 }}>
              <LabeledInput label="End Time" type="time" value={sessionForm.endTime} onChange={(e) => setSessionForm({ ...sessionForm, endTime: e.target.value })} />
            </div>
            <div style={{ flex: 1 }}>
              <LabeledInput
                label="Date"
          ...
          <p style={{ fontSize: 11.5, color: T.muted, margin: "-6px 0 16px" }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
            <button onClick={() => setShowSessionModal(false)} style={{ flex: 1, padding: "11px 0", borderRadius: 10, border: `1px solid ${T.border}`, background: T.cream, color: T.ink, fontWeight: 600, fontSize: 14, cursor: "pointer" }}>
              Cancel
            </button>
            <button onClick={saveSession} style={{ flex: 1, padding: "11px 0", borderRadius: 10, border: "none", background: T.terra, color: T.white, fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
